...
subroutine Evaluate_Mixture(MolecularDesign,FinalStruct,MS,NC,Top,BPSolute,icomp,NSWIP,NSBI,ACEPTABLE,&
           ListPerformance,ent)
!-------------------------------------------------
!   Descripci�n: Calcula propiedades solventes
!   - Variables de entrada
!       compound(10,2):componente
!       mop:    operaci�n de separaci�n (extracci�n L-L(1); destilaci�n extractiva(2))
...
!   SENTENCIAS
   ...
   ! write(*,*)ms(3,:,:)
    ...
    enddo
!   C�lculo y evaluaci�n de peso molecular    
    MWT = MolecularWeight (MS(NC,:,:))    
    if (INV) call MoldesInvertido (MS)
!   C�lculo propiedades compuesto puro    
...
!   C�lculo propiedades de mezcla    
    ...
    if (MolecularDesign .and. NOACE) then
        NSWIP=NSWIP+1 !n�mero de solventes sin par�metros de interacci�n
        return
    endif
!   Evaluaci�n de propiedades de mezcla    
...
!-------------------------------------------------
!   Descripci�n: Calcula propiedades f�sicas de compuesto puro
!   - Variables de entrada
...
!-------------------------------------------------
    ...
    implicit none
!   VARIABLES DE ENTRADA
    integer,intent(in)::ICOMP !ICOMP deber�a sacarse cuando se elimine la evaluaci�n de propiedades de compuesto puro
...
!   C�lculo y evaluaci�n de peso molecular    
    ...
    if (LMW(2).and.(MWT>MWB(2)))then
        nsew=nsew+1 !n�mero de estructuras con alto peso molecular
        return
    endif 
